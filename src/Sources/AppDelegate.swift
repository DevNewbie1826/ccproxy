...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    private func setupMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(NSMenuItem(title: "About CCProxy", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: ""))
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(NSMenuItem(title: "Quit CCProxy", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    ...
    func setupMenuBar() {
        ...
        if let button = statusItem.button {
            if let icon = IconCatalog.shared.image(named: "icon-inactive.png", resizedTo: NSSize(width: 18, height: 18), template: false) {
                button.image = icon
            } else {
                let fallback = NSImage(systemSymbolName: "network.slash", accessibilityDescription: "CCProxy")
            ...
            serverManager.start { [weak self] success in
                DispatchQueue.main.async {
                    if success {
                        ...
                        self?.showNotification(title: "Server Started", body: "CCProxy is now running")
                    } else {
    ...
    @objc func openDashboard() {
        if let url = URL(string: "http://localhost:8328/management.html") {
    ...
    @objc func updateMenuBarStatus() {
        ...
        if let button = statusItem.button {
            ...
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: false) {
                button.image = icon
    ...
    func showNotification(title: String, body: String) {
        ...
        let request = UNNotificationRequest(
            identifier: "com.devnewbie1826.ccproxy.\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
