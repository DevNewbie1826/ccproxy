...
APP_NAME="CCProxy"
BUNDLE_ID="com.devnewbie1826.ccproxy"
...
cp "$BUILD_DIR/CCProxy" "$APP_DIR/Contents/MacOS/"
chmod +x "$APP_DIR/Contents/MacOS/CCProxy"

# Add rpath for Frameworks directory (needed for Sparkle)
install_name_tool -add_rpath "@loader_path/../Frameworks" "$APP_DIR/Contents/MacOS/CCProxy" 2>/dev/null || true
...
# Inject version from environment or clean repo baseline
...
APPCAST_URL="https://raw.githubusercontent.com/DevNewbie1826/ccproxy/main/appcast.xml"
if [ "$TARGET_ARCH" = "x86_64" ]; then
    APPCAST_URL="https://raw.githubusercontent.com/DevNewbie1826/ccproxy/main/appcast-x86_64.xml"
...
if [ -n "$CODESIGN_IDENTITY" ]; then
    ...
    codesign --remove-signature "$APP_DIR/Contents/MacOS/CCProxy" 2>/dev/null || true
    ...
    codesign --force --sign "$CODESIGN_IDENTITY" --options runtime --timestamp "$APP_DIR/Contents/MacOS/CCProxy"
